<?php
	...
	/*=============================================
	* MIENTRAS LA CANTIDAD DE INSPECCIONES A ASCENSORES MODIFICADAS SEA MENOR O IGUAL A CERO LA EJECUCION SE QUEDA EN EL CICLO WHILE 
	...
	*==============================================*/
	while( $count_ascensores <= 0 ){	
		$sql = "SELECT COUNT(*) count_ascensores FROM auditoria_inspecciones_ascensores WHERE k_codusuario=".$codigo_inspector." AND o_actualizar_inspeccion='Si'";
		$result = mysqli_query($con, $sql);
		$row=mysqli_fetch_array($result);
		
		usleep(100000); //Retrasar la ejecucion por (1) segundo
		clearstatcache(); //Borrar Cache 
		$count_ascensores = $row['count_ascensores'];
		if ($time > 30) {
	...
	*==============================================*/
	...
	while($row = mysqli_fetch_array($result)){ 
	    ...
	    $actualizar_bd[] = array('count_ascensores'=> $count_ascensores);
	}
