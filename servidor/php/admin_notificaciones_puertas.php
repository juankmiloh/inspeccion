<?php
  ...
  *==============================================*/
  ...
  $puertas_cantidad_items_nocumple=0;
  while($row = mysqli_fetch_array($result)){
    $cantidad_dias = $row['cantidad_dias'];
    if ($cantidad_dias >= 150 && $cantidad_dias <= 180) {
      $puertas_cantidad_items_nocumple += 1;
    }
  }

  $notificacion_puertas[] = array('id'=> 'puertas_cantidad_items_nocumple',
                                     ...
                                     'cantidad'=> $puertas_cantidad_items_nocumple);
  ...
  *==============================================*/
  $sql="SELECT *,DATEDIFF(now(),f_carga_servidor) AS cantidad_dias FROM auditoria_inspecciones_puertas WHERE v_item_nocumple>0 AND f_carga_servidor<>'0000-00-00 00:00:00' ORDER BY cantidad_dias";
  $result=mysqli_query($con, $sql);
  $puertas_cantidad_items_nocumple_vencidos = 0;
  while($row = mysqli_fetch_array($result)){
    $cantidad_dias = $row['cantidad_dias'];
    if ($cantidad_dias > 180) {
      $puertas_cantidad_items_nocumple_vencidos += 1;
    }
  }

  $notificacion_puertas[] = array('id'=> 'puertas_cantidad_items_nocumple_vencidos',
                                     ...
                                     'cantidad'=> $puertas_cantidad_items_nocumple_vencidos);
