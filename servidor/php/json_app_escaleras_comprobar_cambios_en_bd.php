<?php
	...
	/*=============================================
	* MIENTRAS LA CANTIDAD DE INSPECCIONES A escaleras MODIFICADAS SEA MENOR O IGUAL A CERO LA EJECUCION SE QUEDA EN EL CICLO WHILE 
	...
	*==============================================*/
	while( $count_escaleras <= 0 ){	
		$sql = "SELECT COUNT(*) count_escaleras FROM auditoria_inspecciones_escaleras WHERE k_codusuario=".$codigo_inspector." AND o_actualizar_inspeccion='Si'";
		$result = mysqli_query($con, $sql);
		$row=mysqli_fetch_array($result);
		
		usleep(100000); //Retrasar la ejecucion por (1) segundo
		clearstatcache(); //Borrar Cache 
		$count_escaleras = $row['count_escaleras'];
		if ($time > 30) {
	...
	*==============================================*/
	...
	while($row = mysqli_fetch_array($result)){ 
	    ...
	    $actualizar_bd[] = array('count_escaleras'=> $count_escaleras);
	}
