...
function mostrarDetalleFoto(nombre_foto){
    ...
    largeImage.src="../ascensores/inspector_"+codigo_inspector+"/fotografias/"+codigo_inspeccion+"/"+nombre_foto+"";
...
function eliminarFotografia(btn){
  ...
  $.ajax({
    url: "ascensor_eliminar_valores_fotografias.php",
    data: parametros,
    type: "POST",
    dataType : "html",
    success: function(response){
      ...
      if (response == 0) {
        ...
        location.href="ascensor_fotografias_inspeccion.php?id_inspector="+codigo_inspector+"&cod_inspeccion="+codigo_inspeccion+"";
      }else{
...
function crearPDF(){
  ...
  $.post('ascensor_crear_pdf_inspeccion.php',{
    ...
    if (e == 0) {
      ...
      myWindow = window.open('http://192.168.0.30:8888/inspeccion/servidor/ascensores/servidor/inspector_'
                  +codigo_inspector
                  +'/registros_pdf/'
...
function descargar_fotos(argument) {
  var codigo_inspector = getQueryVariable('id_inspector');
  var codigo_inspeccion = getQueryVariable('cod_inspeccion');
  $.post('ascensor_comprimir_fotografias.php',{
    codigo_inspector: codigo_inspector,
    codigo_inspeccion: codigo_inspeccion
  },function(e){
    location.href="../php/inspeccion_ascensores_"+codigo_inspector+"_"+codigo_inspeccion+".zip";
  });
}
